...
class ViTHyper(nn.Module):
    def __init__(self, n_nodes, embedding_dim, hidden_dim, dim, client_sample, heads=8, dim_head=64, n_hidden=1, depth=6,
                approx=False, hn_head_len=2, hn_hidden_size=512):
        super(ViTHyper, self).__init__()
        self.dim = dim
        self.inner_dim = dim_head * heads
        self.depth = depth
        ...
        # Hypernetwork layers
        self.to_qkv_value_list = nn.ModuleList([
            nn.Linear(hidden_dim, self.dim * self.inner_dim * 3) for _ in range(self.depth)
        ])
    def finetune(self, emd):
        features = self.mlp(emd)
        weights = OrderedDict()

        for d, layer_d_qkv_value_hypermaml in enumerate(self.to_qkv_value_list):
            weights[f"transformer.layers.{d}.0.fn.to_qkv.weight"] = F.normalize((layer_d_qkv_value_hypermaml(features).view(self.inner_dim * 3, self.dim)), p=2, dim=-1)
    ...
    def forward(self, idx, test):
        ...
        if test:
            ...
            weights = OrderedDict()
            for d, layer_d_qkv_value_hypermaml in enumerate(self.to_qkv_value_list):
                weights[f"transformer.layers.{d}.0.fn.to_qkv.weight"] = layer_d_qkv_value_hypermaml(features).view(self.inner_dim * 3, self.dim)
        else:
            # Training mode (MAML-style)
            weights = []
            for _ in range(self.client_sample):
                weights.append(self.finetune(emd))
            for _ in range(self.client_sample):
                 weight_client = OrderedDict()
                 for d, layer_d_qkv_value_hypermaml in enumerate(self.to_qkv_value_list):
                     weight_client[f"transformer.layers.{d}.0.fn.to_qkv.weight"] = layer_d_qkv_value_hypermaml(features).view(self.inner_dim * 3, self.dim)
                 weights.append(weight_client)

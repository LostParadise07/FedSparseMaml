...
class Attention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.1, sparsity_factor=0.5):
        super().__init__()
        inner_dim = dim_head * heads
        self.heads = heads
        self.scale = dim_head ** -0.5
    ...
    def forward(self, x):
        ...
        num_q = q.shape[-2]
        num_k = k.shape[-2]
        num_nonzero_k = int(num_k * self.sparsity_factor)
        ...
        # Apply a sparsity mask
        attn_mask = torch.zeros_like(attn)
        _, topk_indices = torch.topk(attn, num_nonzero_k, dim=-1)
        attn_mask.scatter_(-1, topk_indices, 1.0)

        # Apply attention mask and softmax again
        masked_attn = attn * attn_mask

        # Apply attention to value vectors
        # out = torch.matmul(masked_attn, v)
        out = torch.matmul(masked_attn, v)
        out = rearrange(out, 'b h n d -> b n (h d)')
